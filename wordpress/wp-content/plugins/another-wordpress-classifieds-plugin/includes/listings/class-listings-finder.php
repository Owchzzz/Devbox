...
class AWPCP_ListingsFinder {
    ...
    private function build_where_clause( $query ) {
        ...
        $conditions = array(
		  ...
		  $this->build_county_condition( $query )
        );
	...
	private function build_county_condition( $query ) {
		global $wpdb,$post;
		$conditions = array();
		if(!isset($query['county_id'])) {
			$table = $wpdb->prefix.'tc_county';
			$data = $wpdb->get_row("SELECT * FROM {$table} WHERE post_id={$post->ID}",ARRAY_A);
			$query['county_id'] = $data['id'];
		}
		if($query['county_id']) {
			$conditions[] = $this->build_condition_with_in_clause('listings.`county_id`',$query['county_id']);	
		}
