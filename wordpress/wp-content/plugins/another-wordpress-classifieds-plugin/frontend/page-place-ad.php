...
class AWPCP_Place_Ad_Page extends AWPCP_Page {
    ...
    public function __construct($page='awpcp-place-ad', $title=null, $county_id=null) {
	    if($page == 'default') {
			$page = 'awpcp=place-ad';    
	    }
	    if($title == 'default') {
	...
	protected function get_county_id() {
		global $wpdb,$post;
		$postID = $post->ID;
		$table = $wpdb->prefix . 'tc_county';
		if($data = $wpdb->get_row("SELECT * FROM {$table} WHERE post_id={$postID}",ARRAY_A)) {
			return $data['id'];	
		}
    ...
    public function save_details_step($transaction, $errors=array()) {
        ...
        if ( !$transaction->get('ad-id') || $this->verify_preview_hash($ad) ) {
		  ...
		  if($ad->county_id = $this->get_county_id()) { // Custom
			  
		   }
		   else {
			die('Unable to add to nonexistent county.');   
		   }
            if (!$ad->save()) {
