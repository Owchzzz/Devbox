...
class AWPCP_Pages {
	...
	public function nopp() {
		global $wpdb;
		
		$dbname = $wpdb->prefix.'awpcp_payments';
		$dbname2 = $wpdb->prefix.'awpcp_ads';
		//Paypal thankyou
		$postdata = $this->fetch_postdata();
		if(isset($postdata['custom'])) {
			$id = $postdata['custom'];
			$payment = $wpdb->get_row("SELECT * FROM {$dbname} WHERE id='{$id}'",ARRAY_A); 
			foreach($payment as $key=>$val){
				$payment[$key] = maybe_unserialize($val);	
			}
			if($this->success_ad_payment($dbname,$payment['id'])) {
				if($this->reflect_ad_payment(true,$dbname2,$payment)) {
					echo 'Successsfully completed payment.';	
				}
			}
			else {
				if($this->fail_ad_payment($dbname,$payment['id'])) {
					$this->reflect_ad_payment(false,$dbname2,$payment);
					echo 'Unable to complete payment. please try again later.';	
				}
				else $this->ad_show_completed();
			}
		}
	}
	
	private function ad_show_completed() {
	...
	private function success_ad_payment($db,$payment_id) {
		if(!$this->valid_payment_check($db,$payment_id)) return false;
		global $wpdb;
	...
	private function reflect_ad_payment($status,$db,$payment) {
		global $wpdb;
		if($status) { // Is completed transaction
			...
			$item = $payment['data'];
				...
				$retval = $wpdb->update($db,$updarray,array('ad_id' => $adid));
				return true;
	...
	private function fail_ad_payment($db,$ad_id) {
		if(!$this->valid_payment_check($db,$ad_id)) return false;
		global $wpdb;
		$updarray = array(
	...
	private function fetch_postdata() {
		if(isset($_POST)) {
			$postdata = array();
			foreach($_POST as $key=>$val) {
				$postdata[mysql_real_escape_string($key)] = mysql_real_escape_string($val);
	...
	public function place_ad() {
		...
		if ( ! isset( $this->place_ad_page ) ) {
			global $post;
			$db = $wpdb->prefix.'tc_county';
			$sql = "SELECT * FROM {$db} WHERE post_id={$post->ID}";
			$data = $wpdb->get_row($sql,ARRAY_A);
			$this->place_ad_page = new AWPCP_Place_Ad_Page('default','default',$data['id']);
        	}
...
function awpcpui_homescreen($args) {
	...
	if (!isset($classicontent) || empty($classicontent)) {
		$classicontent=awpcpui_process($awpcppagename,array('county_id'=>$county_id));
	}
	return $classicontent;
}


function awpcpui_process($awpcppagename,$args = array()) {
