...
class AWPCP_BrowseAdsPage extends AWPCP_Page {
    ...
    public function dispatch($args = array()) {
	    ...
	    if($args['county_id'] != -1)  return $this->_dispatch($args);
	    else return 'Invalid County Id.';
    }

    protected function _dispatch($args = array()) {
        ...
        switch ($action) {
            case 'browsecat':
                return $this->browse_listings( 'render_listings_from_category' , $args['county_id'] );
            case 'browseads':
            default:
                return $this->browse_listings( 'render_all_listings' , $args['county_id'] );
        }
    }

    protected function browse_listings( $callback, $county ) {
        ...
        if ( is_null( $output ) ) {
            return $this->$callback( $category_id , $county);
        } else {
            return $output;
        }
    }

    private function render_listings_from_category( $category_id , $county) {
        $query = array(
    ...
    protected function render_all_listings($cat,$county) {
        $query = array(
            'context' => 'public-listings',
            'limit' => absint( awpcp_request_param( 'results', get_awpcp_option( 'adresultsperpage', 10 ) ) ),
            'offset' => absint( awpcp_request_param( 'offset', 0 ) ),
